...
namespace nanoipc {
	...
	public:
		...
		using ReadEncodedFrameFn = std::function<std::optional<std::vector<std::uint8_t>>(ReadBuffer*)>;
		// Responsible for calling read_encoded_frame (passed as first arg) with the buffer until a frame
		// arrives or a timeout is reached. Returns the frame on success, nullopt on timeout.
		using FrameReader = std::function<std::optional<std::vector<std::uint8_t>>(ReadEncodedFrameFn, ReadBuffer*)>;
		...
		NanoIpcClient(
			...
			const FrameReader& frame_reader
		): m_request_serializer(request_serializer), m_response_parser(response_parser), m_serial_data_writer(serial_data_writer), m_read_buffer(read_buffer), m_frame_reader(frame_reader) {
			if (!m_request_serializer || !m_response_parser || !m_serial_data_writer || !m_read_buffer || !m_frame_reader) {
		...
		IpcResponse send(const IpcRequest& request) {
			...
			const auto encoded_frame = m_frame_reader(read_encoded_frame, m_read_buffer);
